...
public class AiEnemy extends Tank {
    ...
    public AiEnemy(World world, String id, int x, int y, CollisionManager collisionManager) {
        super(world, id, Math.random() < 0.5 ?  "tank.png" : "tank2.png", x, y, Type.SMART_PLAYER, collisionManager);

        boundsPoly.scale(-0.5f);
        getSprite().scale(-0.5f);
        birdAnimation = new Animation(new TextureRegion(Assets.getInstance().getManager().get(textureFileName, Texture.class)), 3, 0.5f);
